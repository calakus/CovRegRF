...
void registerCustomFunctions(void) {
  ...
  registerThis (&sampleCovEuclideanDist, REGR_FAM, 2);
}
...
double sampleCovEuclideanDist (unsigned int n,
                               char        *membership,
                               double      *time,
                               double      *event,
                               ...
                               unsigned int featureCount)
{
    ...
    if( (leftSize > dimY) && (rghtSize > dimY) ){
        ...
        for (row = 1; row <= n; row++) {
            if(membership[row] == LEFT) {
                for(col = 1; col <= dimY; col++){
                    leftY[(col-1)*leftSize + ixLeft] = feature[col][row];
                }
                ixLeft++;
            }
            else{
                for(col = 1; col <= dimY; col++){
                    rightY[(col-1)*rghtSize + ixRight] = feature[col][row];
                }
                ixRight++;
            }
        }

        // Mean centering the response matrices in the left and right nodes
        for (col = 0; col < dimY; col++) {
            colSumL = 0.00;
            for (row = 0; row < leftSize; row++) {
                colSumL += leftY[col*leftSize+row];
            }
            colMeanL = colSumL / leftSize;
            for (row = 0; row < leftSize; row++) {
                leftY[col*leftSize+row] -= colMeanL;
            }

            colSumR = 0.00;
            for (row = 0; row < rghtSize; row++) {
                colSumR += rightY[col*rghtSize+row];
            }
            colMeanR = colSumR / rghtSize;
            for (row = 0; row < rghtSize; row++) {
                rightY[col*rghtSize+row] -= colMeanR;
            }
        }
        ...
        for (i = 0; i < dimY; i++) {
            for (j = i; j < dimY; j++) {
                dist += pow( ( (leftCov[i*dimY+j]/(leftSize-1)) - (rghtCov[i*dimY+j]/(rghtSize-1)) ), 2);
            }
        }
        dist = sqrt(dist);
