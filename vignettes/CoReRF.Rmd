---
title: "CoReRF: Covariance Regression with Random Forests"
output: 
  rmarkdown::html_vignette:
    number_sections: true
vignette: >
  %\VignetteIndexEntry{CoReRF: Covariance Regression with Random Forests}
...
Firstly, we check the global effect of $\mathbf{X}$ by applying the significance test for the three covariates.
```{r}
...
Next, we apply the proposed method with `corerf()` and get the out-of-bag (OOB) covariance matrix estimates for the training observations.
```{r}
corerf.obj <- corerf(formula, traindata, params.rfsrc = list(ntree = 200))
...
vimp.obj <- vimp(corerf.obj)
vimp.obj$importance
plot.vimp(vimp.obj)
```

From the VIMP measures, we see that $X_3$ has a smaller importance than $X_1$ and $X_2$. Therefore, we apply the significance test to evaluate the effect of $X_3$ on the covariance matrices while controlling for $X_1$ and $X_2$.
```{r}
partialsig.obj <- significance.test(formula, traindata, params.rfsrc = list(ntree = 200), 
                                    nperm = 10, test.vars = "x3")
partialsig.obj$pvalue
```
Using 10 permutations, the estimated *p*-values with is 0.3 and we fail to reject the null hypothesis, indicating that we do not have enough evidence to prove that $X_3$ has an effect on the estimated covariance matrices while $X_1$ and $X_2$ are in the model. Using 500 permutations, the estimated $p$-value is 0.218.

Finally, we can get the covariance matrix predictions for the test observations. 
```{r}
pred.obj <- predict(corerf.obj, testdata)
pred <- pred.obj$predicted
head(pred, 2)
```

# References
Alakus, C., Larocque, D., and Labbe, A. (2022). Covariance regression with random forests. arXiv ....
